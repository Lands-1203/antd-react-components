...
export default () => {
  const [deptList, setDeptList] = useState<Record<string, any>[]>([]);
  const [deptTree, setDeptTree] = useState<Record<string, any>[]>([]);
  const [deptId, setDeptId] = useState('');

  useEffect(() => {
    const data = getMockData();
    const tree: Record<string, any>[] = listToTree({
      data,
      id: 'deptId',
      F: {
  ...
  return (
    deptId && (
      <SearchTree
        ...
        dataTree={deptTree as SearchTreeDataProps[]}
        dataList={deptList as SearchListDataProps[]}
